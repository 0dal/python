...
сообщение о превышении скорости. Создайте экземпляры классов, передайте
значения атрибутов. Выполните доступ к атрибутам, выведите результат.
...
def drive(avto):
    print(f"Движение {'полицейского' if avto.is_police else 'гражданского'} "
          f"автомобиля {avto.name}, цвет {avto.color}")
    avto.go(40)
    avto.show_speed()
    avto.turn('направо')
    avto.stop()
...
car = Car('синий', 'Ниссан', False)
drive(car)
mers = TownCar('черный', 'Мерседес')
drive(mers)
subaru = SportCar('желтый', 'Субару')
drive(subaru)
gazel = WorkCar('красный', 'Газель')
drive(gazel)
mustang = PoliceCar('синий', 'Мустанг')
drive(mustang)
